-- Script final pour ajouter des données de test
...
-- Messages Conversation 1 : Dr Martin et Marie Dupont
INSERT INTO messages (conversation_id, sender_id, content, is_read, created_at)
SELECT c.id, '44444444-4444-4444-4444-444444444444', 'Bonjour Docteur, j''ai une question concernant mon ordonnance.', true, NOW() - INTERVAL '2 days'
FROM conversations c WHERE c.medecin_id = '11111111-1111-1111-1111-111111111111' AND c.patient_id = '44444444-4444-4444-4444-444444444444'
ON CONFLICT DO NOTHING;

INSERT INTO messages (conversation_id, sender_id, content, is_read, created_at)
SELECT c.id, '11111111-1111-1111-1111-111111111111', 'Bonjour Marie, je vous écoute. Que souhaitez-vous savoir ?', true, NOW() - INTERVAL '2 days' + INTERVAL '1 hour'
FROM conversations c WHERE c.medecin_id = '11111111-1111-1111-1111-111111111111' AND c.patient_id = '44444444-4444-4444-4444-444444444444'
ON CONFLICT DO NOTHING;

INSERT INTO messages (conversation_id, sender_id, content, is_read, created_at)
SELECT c.id, '44444444-4444-4444-4444-444444444444', 'Dois-je prendre le paracétamol avant ou après les repas ?', true, NOW() - INTERVAL '2 days' + INTERVAL '2 hours'
FROM conversations c WHERE c.medecin_id = '11111111-1111-1111-1111-111111111111' AND c.patient_id = '44444444-4444-4444-4444-444444444444'
ON CONFLICT DO NOTHING;

INSERT INTO messages (conversation_id, sender_id, content, is_read, created_at)
SELECT c.id, '11111111-1111-1111-1111-111111111111', 'Vous pouvez le prendre indifféremment, mais après les repas est préférable pour éviter les irritations gastriques.', false, NOW() - INTERVAL '2 days' + INTERVAL '2 hours' + INTERVAL '15 minutes'
FROM conversations c WHERE c.medecin_id = '11111111-1111-1111-1111-111111111111' AND c.patient_id = '44444444-4444-4444-4444-444444444444'
ON CONFLICT DO NOTHING;

-- Messages Conversation 2 : Dr Dubois et Thomas Leroy
INSERT INTO messages (conversation_id, sender_id, content, is_read, created_at)
SELECT c.id, '22222222-2222-2222-2222-222222222222', 'Bonjour Thomas, voici votre ordonnance pour le traitement de l''hypertension.', true, NOW() - INTERVAL '1 day'
FROM conversations c WHERE c.medecin_id = '22222222-2222-2222-2222-222222222222' AND c.patient_id = '55555555-5555-5555-5555-555555555555'
ON CONFLICT DO NOTHING;

INSERT INTO messages (conversation_id, sender_id, content, is_read, created_at)
SELECT c.id, '55555555-5555-5555-5555-555555555555', 'Merci Docteur. Combien de temps dois-je suivre ce traitement ?', true, NOW() - INTERVAL '1 day' + INTERVAL '3 hours'
FROM conversations c WHERE c.medecin_id = '22222222-2222-2222-2222-222222222222' AND c.patient_id = '55555555-5555-5555-5555-555555555555'
ON CONFLICT DO NOTHING;

INSERT INTO messages (conversation_id, sender_id, content, is_read, created_at)
SELECT c.id, '22222222-2222-2222-2222-222222222222', 'C''est un traitement de fond à prendre quotidiennement. Nous ferons un point dans 3 mois.', false, NOW() - INTERVAL '1 day' + INTERVAL '4 hours'
FROM conversations c WHERE c.medecin_id = '22222222-2222-2222-2222-222222222222' AND c.patient_id = '55555555-5555-5555-5555-555555555555'
ON CONFLICT DO NOTHING;

-- Messages Conversation 3 : Dr Bernard et Emma Moreau
INSERT INTO messages (conversation_id, sender_id, content, is_read, created_at)
SELECT c.id, '66666666-6666-6666-6666-666666666666', 'Bonjour, je voudrais prendre rendez-vous pour ma fille.', true, NOW() - INTERVAL '3 days'
FROM conversations c WHERE c.medecin_id = '33333333-3333-3333-3333-333333333333' AND c.patient_id = '66666666-6666-6666-6666-666666666666'
ON CONFLICT DO NOTHING;

INSERT INTO messages (conversation_id, sender_id, content, is_read, created_at)
SELECT c.id, '33333333-3333-3333-3333-333333333333', 'Bonjour, avec plaisir. Quel est le motif de la consultation ?', true, NOW() - INTERVAL '3 days' + INTERVAL '2 hours'
FROM conversations c WHERE c.medecin_id = '33333333-3333-3333-3333-333333333333' AND c.patient_id = '66666666-6666-6666-6666-666666666666'
ON CONFLICT DO NOTHING;

INSERT INTO messages (conversation_id, sender_id, content, is_read, created_at)
SELECT c.id, '66666666-6666-6666-6666-666666666666', 'C''est pour un contrôle de routine et les vaccins.', true, NOW() - INTERVAL '3 days' + INTERVAL '3 hours'
FROM conversations c WHERE c.medecin_id = '33333333-3333-3333-3333-333333333333' AND c.patient_id = '66666666-6666-6666-6666-666666666666'
ON CONFLICT DO NOTHING;

INSERT INTO messages (conversation_id, sender_id, content, is_read, created_at)
SELECT c.id, '33333333-3333-3333-3333-333333333333', 'Parfait, j''ai une disponibilité mardi prochain à 9h30.', false, NOW() - INTERVAL '3 days' + INTERVAL '3 hours' + INTERVAL '30 minutes'
FROM conversations c WHERE c.medecin_id = '33333333-3333-3333-3333-333333333333' AND c.patient_id = '66666666-6666-6666-6666-666666666666'
ON CONFLICT DO NOTHING;
...
-- Associer consultations aux dossiers médicaux via dossier_id
INSERT INTO public.consultations (medecin_id, dossier_id, date_consultation, motif, examen_clinique, diagnostic, notes, constantes)
SELECT 
  'aaaaaaaa-aaaa-aaaa-aaaa-aaaaaaaaaaaa',
  dm.id,
  NOW() - INTERVAL '7 days',
  'Consultation de suivi',
  'Patient présente fièvre (38.5°C), maux de tête, fatigue générale. Examen physique: gorge légèrement rouge, auscultation pulmonaire normale.',
  'État grippal avec syndrome fébrile',
  'Patient en voie de guérison. Repos recommandé. Revoir si symptômes persistent au-delà de 5 jours. Ordonnance prescrite.',
  '{"temperature": "38.5", "tension": "120/80", "frequence_cardiaque": "75"}' ::jsonb
FROM dossiers_medicaux dm
WHERE dm.patient_id = 'dddddddd-dddd-dddd-dddd-dddddddddddd'
ON CONFLICT DO NOTHING;
...
SELECT 
  ...
  NOW() - INTERVAL '14 days',
  'Contrôle cardiaque',
  'Patient signale tension élevée et légers vertiges. Auscultation cardiaque: bruits normaux, pas de souffle. ECG réalisé (rythme sinusal).',
  'Hypertension artérielle modérée (stade 1)',
  'Traitement antihypertenseur initié (Amlodipine 5mg). Contrôle de la tension dans 1 mois. Régime pauvre en sel et exercice physique modéré conseillés.',
  '{"tension": "145/92", "frequence_cardiaque": "82", "poids": "85"}' ::jsonb
FROM dossiers_medicaux dm
WHERE dm.patient_id = 'eeeeeeee-eeee-eeee-eeee-eeeeeeeeeeee'
ON CONFLICT DO NOTHING;
...
SELECT 
  ...
  'Enfant en bonne santé générale. Examen physique: développement normal pour l''âge. Pas de contre-indication à la vaccination.',
  'Vaccination DTP à jour - Rappel administré',
  'Vaccin DTP administré sans complication. Aucune réaction indésirable observée. Prochain rappel prévu dans 10 ans. Parents informés des éventuels effets secondaires bénins.',
  '{"temperature": "36.8", "poids": "28", "taille": "125"}' ::jsonb
FROM dossiers_medicaux dm
WHERE dm.patient_id = 'ffffffff-ffff-ffff-ffff-ffffffffffff'
ON CONFLICT DO NOTHING;
